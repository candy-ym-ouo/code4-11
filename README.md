...
- 受保护图片附件。
- 批次短码标签：重印（印次）、作废、换签链与扫码解析定位。
- 扫码轨迹、最近出现位置与离线补传（事件幂等、逐条容错、整包可重放）。
- 离线识别按（材料、批次号）自然键幂等建档，重复识别与并发绝不产生重复批次。
- CSV、完整工作区 JSON 导出。
- 仪表盘、审计日志和健康检查。
...
API 集成测试使用 embedded-postgres（真实 PostgreSQL 18 二进制）自动启动临时实例，无需
外部数据库。linux/arm64 首次运行测试会通过 `apps/api/scripts/fetch-test-libs.sh` 下载
所需的 ICU60 运行库到 `apps/api/test-libs/`（已 gitignore）；x64 环境检测到原生依赖齐全
时会自动跳过。
...
- 短码一经签发永不改变、永不回收；重印只增印次，作废只改状态，换签形成可追溯链。
- 扫码与识别以客户端 UUID `eventId` 幂等，解析结论首次入库即冻结，离线补传可任意重放。
- 批次以（材料、批次号）为自然键：离线识别先建 `PENDING` 批次，正式入库激活同一行。
- 归档代替核心数据硬删除。
- 审计日志只追加，不更新、不删除。
